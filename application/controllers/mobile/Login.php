...
class Login extends CI_Controller {
	   ...
	   $id = $this->session->sess_user['t_user_id'];
		...
		if ($id) {
         ...
         $this->session->set_flashdata('hx_info', $pesan);
		  if($this->session->sess_user['t_user_level'] == 'enduser'){
			  redirect('mobile/enduser');
		  }else if($this->session->sess_user['t_user_level'] == 'store'){
			  redirect('mobile/toko');
		  }
		   
      }
      else {
         $this->load->view('mobile/login/login');
      }
	}
   ...
   public function validasi()
   {
      ...
      if ($user)
      {
         ...
         { 
				if ($user['t_user_verifikasi']=='sudah')
				{
					$today_time = strtotime(date('Y-m-d'));
					$expire_time = strtotime($user['t_user_exp']);
					if ($expire_time < $today_time) {
						 $load['error'] = 'Masa aktif akun Anda telah habis.';
						$this->load->view('mobile/login/login',$load);
					}else{
						   ...
						   if($user['t_user_level'] == 'enduser'){
								redirect('mobile/enduser');
						   }else if($user['t_user_level'] == 'store'){
								redirect('mobile/toko');
						   }
					}
			   }else{
                     $load['error'] = 'Anda belum terverifikasi oleh administrator.';
                     $this->load->view('mobile/login/login',$load);
               }
         }
         else
         {
            $load['error'] = 'Password yang anda masukan salah';
            $this->load->view('mobile/login/login',$load);
         }
      }
      else
      {
         $load['error'] = 'Anda belum terdaftar. Silahkan mengubungi Administrator';
         $this->load->view('mobile/login/login',$load);
      }
   }
