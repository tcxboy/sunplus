   ...
   public function set_input($layout,$name,$label,$value=null,$type,$list=null,$lebar=null,$attr=null)
   {
      ...
      switch ($type):
         ...
         case 'select':
            ...
            $input .= $buka_div;
            $input .= '<select name="data['.$name.']" id="'.$name.'" class="form-control select" '.$attr.'>';
      ...
      $form = '<div class="modal-dialog">
                  <div class="modal-content">
                     <form class="hx_form_upload_file" action="'.site_url($arr['url_upload']).'" method="post" enctype="multipart/form-data">
						...
						<div class="modal-header">
							<h5 class="modal-title" id="exampleModalLongTitle"><i class="fa fa-upload fa-fw"></i> Upload '.ucwords(str_replace('_',' ',$field)).'</h5>
							<button type="button" class="close" data-dismiss="modal" aria-label="Close">
							  <span aria-hidden="true">&times;</span>
							</button>
						  </div>
